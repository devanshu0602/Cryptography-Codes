...
def find_index_from_table(playfair_table, char):
    row = next(i for i, row in enumerate(playfair_table) if char in row)
...
def fairplay_cipher_encryption(plain_text, playfair_table):
    ...
    encrypted_text = ""
    for pair in pairs:
        letter_1, letter_2 = pair[0], pair[1]
        # Extract the index of the characters
        letter_1_row, letter_1_col = find_index_from_table(playfair_table=playfair_table, char=letter_1)
        letter_2_row, letter_2_col = find_index_from_table(playfair_table=playfair_table, char=letter_2)
        # Find the encrypting character 
        if letter_1_row == letter_2_row:
            letter_1_col_new = (letter_1_col + 1) % 5
            letter_1_row_new = letter_1_row
            letter_2_col_new = (letter_2_col + 1) % 5
            letter_2_row_new = letter_2_row
        elif letter_1_col == letter_2_col:
            letter_1_row_new = (letter_1_row + 1) % 5
            letter_1_col_new = letter_1_col
            letter_2_row_new = (letter_2_row + 1) % 5
            letter_2_col_new = letter_2_col
        else:
            letter_1_col_new = letter_2_col
            letter_1_row_new = letter_1_row
            letter_2_col_new = letter_1_col
            letter_2_row_new = letter_2_row
